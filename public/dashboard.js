...
function scannerRowHtml(r) {
  ...
  const path = `<div class="path ratio-path"><span><b>BUY LOW</b> ${ratioText(s.buyRatio,s.item.name,s.buyCurrency.name)}</span><span><b>SELL HIGH</b> ${ratioText(s.sellRatio,s.sellCurrency.name,s.item.name)}</span><span><b>RETURN</b> ${ratioText(s.returnRatio,s.buyCurrency.name,s.sellCurrency.name)}</span><small>Target batch: ${fmtInt(s.startingQuantity)} → ${fmtInt(s.finalStartingQuantity)} ${esc(s.buyCurrency.name)}</small></div>`;
...
function scannerDrawer(s) {
  ...
  const ratioStep = (title, ratio, wantName, haveName, batch) => `<li><b>${title}</b><div class="ratio-entry"><small>Enter in Exchange · I WANT : I HAVE</small><strong>${ratioText(ratio,wantName,haveName)}</strong></div><span class="muted">Sizing guide: ${batch}</span></li>`;
