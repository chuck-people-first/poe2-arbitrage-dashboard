...
function scannerRowHtml(r) {
  ...
  const path = `<div class="path ratio-path"><span><b>BUY</b> ${ratioText(s.buyRatio,s.item.name,s.buyCurrency.name)}</span><span><b>SELL</b> ${ratioText(s.sellRatio,s.sellCurrency.name,s.item.name)}</span><span><b>RETURN</b> ${ratioText(s.returnRatio,s.buyCurrency.name,s.sellCurrency.name)}</span><small>Enter these I WANT : I HAVE ratios · ${esc(sourceAgeText(s.sourceHourUtc))} · confirm live</small></div>`;
...
function scannerDrawer(s) {
  ...
  const ratioStep = (title, ratio, wantName, haveName, batch) => `<li><b>${title}</b><div class="ratio-entry"><small>Type this ratio in Exchange · I WANT : I HAVE</small><strong>${ratioText(ratio,wantName,haveName)}</strong></div><span class="muted">Optional batch illustration (do not type this): ${batch}</span></li>`;
