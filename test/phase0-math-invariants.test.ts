...
const valuation=():ValuationDisclosure=>({profitKind:"mark-to-market",inputValuationPath:[{observationId:"x",from:A,to:BASE,rate:2}],outputValuationPath:[{observationId:"y",from:B,to:BASE,rate:2}],observationIds:["x","y"],valuationRates:[2,2],returnToBaseLegs:[],returnToBaseIncluded:false,valuationBottleneckVolumeShare:.01,valuationRangeUncertaintyPct:5,valuationConfidence:.8,valuationExecutable:false,valuationGoldIncluded:false,valuationTradeCountIncluded:0});
const baseRoute=(overrides:Partial<Route>={}):Route=>({id:"route",routeFamilyId:"fam",strategy:"two-leg-cross",startCurrency:A,endCurrency:B,hubCurrency:B,legs:[leg()],startUnits:10,endUnits:20,grossProfitBase:10,inputValueBase:10,goldCostTotal:100,movementHaircutPct:1,ratioRangeUncertaintyPct:5,temporalMovementPct:null,movementStatus:"insufficient-history",estimatedMarketImpactPct:0.5,conservativeProfitBase:9,fillConfidence:.8,expectedProfitBase:9.5,score:1,divineProfitPerGold:0.09,profitPerTrade:9,capitalRoiPct:90,bottleneckVolumeShare:.01,bottleneckEdgeKey:"A-B",dataAgeHours:0,ratioRangePct:5,profitKind:"mark-to-market",profitClass:"mark-to-market",realizedCurrency:null,realizedProfitStart:null,realizedProfitBase:null,valuation:valuation(),...overrides});
...
describe("Phase 0 math invariants",()=>{
 ...
 it("chooses one deterministic lexicographic size but preserves different observed routes",()=>{
   const a=baseRoute({id:"a",divineProfitPerGold:10,profitPerTrade:5,startUnits:100});
   const b=baseRoute({id:"b",divineProfitPerGold:11,profitPerTrade:4,startUnits:100});
 ...
 it("scopes dedup by source hour so identical paths at different hours stay separate",()=>{
   const a=baseRoute({id:"a",divineProfitPerGold:10,profitPerTrade:5});
   const b=baseRoute({id:"b",divineProfitPerGold:11,profitPerTrade:4});
   const sameHour=baseRoute({id:"same",divineProfitPerGold:9});
   ...
   const otherHour=baseRoute({id:"otherhour",divineProfitPerGold:9,startCurrency:"A",endCurrency:"B"});
