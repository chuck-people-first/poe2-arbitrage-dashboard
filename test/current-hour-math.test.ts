...
const settings:RunSettings={startCurrency:GGG_HUB_PATHS.CHAOS,baseCurrency:GGG_HUB_PATHS.DIVINE,capitalUnits:100,goldBudget:2_000_000,maxLegs:3,maxVolumeSharePct:20,minConservativeProfitBase:.05,maxDataAgeHours:0,movementRiskTolerancePct:100};
const payload=parseGggPayload(JSON.parse(readFileSync("fixtures/ggg-currency-exchange-1787090400.json","utf8")));
const edges=deriveEdges(payload.markets.filter(m=>m.league==="Runes of Aldur"),"2026-08-18T22:00:00Z");
...
describe("2026-08-18 22:00 corrected cross-currency math",()=>{
 it("uses the observed first leg for input valuation and keeps profit ordering",()=>{
   const evaluated=evaluateCandidate(candidate);const scored=scoreCandidate(candidate,evaluated,edges,settings);
